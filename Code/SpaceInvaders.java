...
public class SpaceInvaders extends JPanel implements ActionListener, KeyListener {
    ...
    private final int MAX_BULLETS = 3; // Maximum number of bullets allowed on screen
    ...
    private void createInvaders() {
        ...
        int rows = 2 + (wave / 2); // Calculate number of rows (increases slowly with waves)
        int cols = 6; // Fixed number of columns
        ...
        for (int row = 0; row < rows; row++) { // For each row
            for (int col = 0; col < cols; col++) { // For each column
                ...
                int health = postBossMode ? 2 : 1; // Enemies take 2 hits after boss levels
    ...
    private void updateGame() {
        ...
        if (bossLevel && boss != null) { // If in boss level and boss exists
            ...
            if (random.nextInt(100) < 2) { // 2% chance each frame to shoot
        ...
        while (bulletIter.hasNext()) {
            ...
            while (invaderIter.hasNext()) {
                ...
                if (bullet.x >= invader.x && bullet.x <= invader.x + INVADER_SIZE &&
                    ...
                    invader.health--; // Reduce invader health
                    bulletIter.remove(); // Remove bullet
    ...
    public void shoot() {
        if (bullets.size() < MAX_BULLETS) { // Only shoot if fewer than max bullets
            bullets.add(new Bullet(playerX + PLAYER_WIDTH / 2 - BULLET_SIZE / 2, playerY));
        }
    }
    ...
    private void drawAlienEmojiSprite(Graphics g, int x, int y, Color color, int health) {
        ...
        if (health == 2) { // If enemy takes 2 hits
            ...
            g.fillRect(x, y - 5, INVADER_SIZE / 2, 3); // Draw half health (1 hit taken)
        }
    }
    ...
    private void drawHUD(Graphics g) {
        ...
        if (postBossMode) { // If in tough enemy mode
            ...
            g.drawString("TOUGH ENEMIES!", WIDTH / 2 - 80, 30); // Draw warning
        }
    ...
    private void drawGameOverScreen(Graphics g) {
        ...
        g.drawString("Press R to Restart", WIDTH / 2 - 100, HEIGHT / 2 + 60); // Draw restart instructions
    }
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Space Invaders - Final Version"); // Create game window
